...
function sanitizeCallback(raw: string | null | undefined) {
  // Only allow same-origin paths; ignore static assets or empty values
...
export default function LoginPage() {
  ...
  const callbackUrl = useMemo(() => sanitizeCallback(sp.get('callbackUrl')), [sp]);
  ...
  return (
    <div className="min-h-screen bg-black text-white flex items-center justify-center">
      <div className="w-full max-w-sm p-6 rounded-xl border border-neutral-800 bg-neutral-900/50 space-y-4">
        ...
        {/* Google SSO — button triggers NextAuth; link provides hard fallback */}
        <button
          type="button"
        ...
        <div className="text-center text-xs text-neutral-500">
          If the button doesn’t redirect, <a
            className="underline"
